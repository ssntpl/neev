        ...
        <x-neev-component::card class="border border-red-600">
            ...
            <x-slot name="content">
                @if ($team->user_id === $user->id)
                    <x-neev-component::input-error for="message"/>
                    {{-- Change Owner --}}
                    <div x-data="{ show: false, user_id: @js($team->user_id), owner_id: @js($team->user_id) }">
                        ...
                        <x-neev-component::dialog-modal x-show="show" x-cloak @keydown.escape.window="show = false" @click.away="show = false">
                            ...
                            <x-slot name="footer">
                                ...
                                <x-neev-component::button type="submit" x-bind:disabled="user_id === owner_id" class="ms-3" @click.prevent="$refs.changeOwnerForm.submit()">
                                    {{ __('Make Owner') }}
                    ...
                    <div x-data="{ show: false, inputName: '', teamName: @js($team->name) }">
                        ...
                        <x-neev-component::dialog-modal x-show="show" x-cloak @keydown.escape.window="show = false" @click.away="show = false">
                            ...
                            <x-slot name="footer">
                                ...
                                <x-neev-component::danger-button type="submit" x-bind:disabled="inputName !== teamName" class="ms-3" @click.prevent="$refs.deleteTeamForm.submit()">
                                    {{ __('Delete Team') }}
                                </x-neev-component::danger-button>
                            </x-slot>
                        </x-neev-component::dialog-modal>
