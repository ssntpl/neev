...
- **An OAuth provider can no longer pass for a built-in login method** — the callbacks wrote the bare provider name from `config('neev.oauth')` into `login_attempts.method`, the same column whose values `NeevMiddleware` reads to decide which first factors answer for MFA on their own. A provider named `sso` therefore inherited the tenant-SSO exemption and carried an MFA-enrolled account straight past its challenge; `passkey` would have done the same. Provider names are configuration, so the collision needed nothing more than a listing. An OAuth login is now recorded as `oauth:{provider}`. No migration ships: rows written before the upgrade keep the bare name, and [UPGRADING.md](./UPGRADING.md) has the backfill for anything that queries them
...
- **The web OAuth callback mails the first email MFA code** — it parked an email-MFA account at the challenge and relied on the Blade challenge page to issue the code when it opened. A headless install has no page of ours, so the account had nothing to answer with. The callback now issues and mails the code itself, as the API callback always did. Every email-MFA send goes through `AuthService::sendMfaEmailCode()` — the Blade challenge page, the API login and the OAuth callback alike — and the helper leaves a live code alone, so under the kit a first factor that already mailed a code does not mail a second, and reopening the challenge page no longer refreshes the window on one six-digit secret. The two deliberate resends (`POST {prefix}/mfa/otp/send` and the Blade `otp.mfa.send`) pass `force: true`, since there the user is telling us the first mail never arrived
...
- **`magic_link_tokens` indexes match how it is queried** — `expires_at` is now indexed: `neev:clean-magic-links` and `MagicLinkToken::scopeActive()` both range over it, and unindexed the cleaner full-scanned, slowest exactly when the table was largest and while contending with the insert every link request makes. The separate `user_id` index is dropped — `['user_id', 'channel']` covers it by leftmost prefix, so carrying both only added write cost on that same hot path. Dropping the declaration is not enough on MySQL, which creates an index of its own for a foreign key that has none: Laravel emits a `constrained()` foreign key ahead of any index declared after it, so the composite did not yet exist when the constraint landed and MySQL added `magic_link_tokens_user_id_foreign` anyway — the saving was real on PostgreSQL and SQLite only. The column, its indexes and then its constraints are now declared in that order, so MySQL finds the composite already covering `user_id` and adopts it.
...
- **An unreachable channel is refused before the send costs anything** — `MagicLinkManager::generate()` built the link's URL only after committing, so a request for a deep-link channel with no `scheme`/`universal_link` (`channel=mobile` with `NEEV_MOBILE_SCHEME` unset) threw its `422` having already deleted the user's previous mobile link, stored a token nobody could reach, fired `MagicLinkGenerated` and spent one of the account's three issuances. The channel's base URL is now resolved before the issuance is reserved and before the transaction, alongside the binding check, so a refused send costs the user neither their live link nor their budget
- **Redeeming a link gives the issuance budget back** — the per-account cap counted issuances but nothing cleared them on redemption, so a user who legitimately requested and used three links inside the five-minute window (a second device, a lost mail, a re-login after logout) was hard-refused for the rest of it. The cap exists to bound how often an *unconsumed* link can be replaced out from under its owner; a successful `consume()` ends that, and now clears the account's counter for that channel
