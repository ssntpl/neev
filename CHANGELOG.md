...
- **Lifecycle events** — `MagicLinkGenerated`, `MagicLinkConsumed`, `MagicLinkRejected` for auditing, notifications, and abuse detection. Each describes the token with scalars (`$tokenId`, `$channel`, `$expiresAt`, plus `$createdIp`/`$userAgent` on generation) rather than carrying the `MagicLinkToken` model, so they are safe to handle from a queue. Carrying the model failed both ways: as a top-level property `SerializesModels` restores it with `firstOrFail()`, and the row is deleted on redemption — so a listener queued behind a user who clicked promptly threw `ModelNotFoundException`, losing exactly the audit records generated under load. Reachable any other way (as `MagicLinkResult` held it) a model is serialized by value instead, which put the account's bcrypt password hash and the stored token hash into the queue payload — and so into Redis, `jobs`, and `failed_jobs`, which is retained indefinitely and routinely copied into backups and staging. `MagicLinkRejected` was the sharpest case: it fires on `BINDING_MISMATCH` without deleting the row, so its payload carried the stored hash of a still-live token. `$user` remains a model, which `SerializesModels` reduces to a class-and-id reference.
...
- **Passkeys work on a tenant's own domain** — the relying party ID was one app-wide config value, and WebAuthn requires it to be the browser origin's host or a registrable suffix of it, so a tenant reaching the app on `acme.com` could not use passkeys at all: the browser refused the ceremony before a request was made, whatever the domain's verification status. It is now read off the request by one rule — the **verified domain equal to the request's origin**, whether that is the row the request resolved through or one the resolved tenant/team owns, matched exactly, so a row covers the host it names and no other — and recorded against each credential in the new `passkeys.rp_id` column, since a passkey is bound to exactly one relying party for its lifetime. Login offers and accepts only the credentials belonging to the current one.
  The origin is the browser's `Origin` header and only that — the request's host is the host the request was addressed to, not where the ceremony would run. Browsers attach `Origin` themselves on every POST and on every cross-origin request, and omit it on a same-origin GET, where it is a forbidden header name a client cannot add back — so **both API options endpoints are now `POST`** (see Changed below), and name their origin wherever they are called from. A caller that legitimately has no host origin — a native app, whose origin is an app facet — keeps the configured relying party, the only one it can hold platform assets for. The context is whichever one the request resolved — the `X-Tenant` header, a subdomain, a custom domain — so the API may be deployed anywhere: a SPA on `acme.com` calling an API on `api.platform.com` sends `X-Tenant`, as it must for everything else to be scoped correctly, and the ceremony runs under `acme.com`. A domain inside the platform's own zone keeps the platform relying party, so a verified subdomain tenant does not displace it and retire the passkeys already enrolled there. Nor does a domain the origin cannot use: `domains` is also the federation registry, so a team reached at `acme.example.com` may hold a verified `acme.com` purely so that `@acme.com` staff auto-join, with nothing served there — handing that row the relying party would have made `navigator.credentials.create()` throw `SecurityError` on the host users sign in on and dropped every enrolled credential out of `allowCredentials`. Only verified rows count and the row is read on every ceremony, so a domain losing its verification stops granting a relying party on the very next request — the credential rows survive and are simply never selected again, with nothing deleted on a user's behalf.
  **A tenant gets one relying party per origin it is reached on.** A tenant holding domains with different registrable roots gets `ssntpl.in` on `ssntpl.in` and `otper.com` on `otper.com`, with a separate credential on each — no credential can span the two, whoever owns them. Subdomains are domains like any other: a verified `app.acme.com` is its own relying party with its own credentials, and an unverified one keeps `relying_party_id`, which its browser then refuses — so verify every host users sign in on. Where passkeys are needed on more than one of a tenant's roots, the application chooses how — redirecting to the primary to sign in, serving `/.well-known/webauthn` for Related Origin Requests, or giving each domain its own context; the docs cover all three.
  A ceremony admits one origin from the `domains` table — built from the record rather than the request, with no subdomain matching: the custom domain the relying party was taken from, or, inside the platform's own zone, only the single verified host the request names (never every platform-zone row the context holds, since those share one relying party and a sibling would otherwise complete a ceremony against a credential enrolled here) — added to the configured `allowed_origins`, which is kept whole on every relying party so a native app's facet (`android:apk-key-hash:…`) listed once there keeps working on tenant domains. `allowed_origins` no longer has to list custom domains, and the host you serve passkeys from is the host to verify. The challenge is stored together with the relying party it was issued for, so the options and completion halves of a ceremony cannot run under different contexts. Registration options list the user's credentials on that relying party in `excludeCredentials`, so an authenticator refuses to enrol the same key twice, and `rp.name` is the domain owner's name on its own domain (the app name on the platform). With both tenants and teams on, a team-owned host routes through that team's tenant, so the tenant holds no row naming it — the resolving row is taken anyway, since it is the host the browser is on, and `rp.name` there is the owning team's. Existing installs are untouched: the configured value still covers its own domain and every subdomain, credentials with a null `rp_id` are read as it, and a challenge issued before the upgrade — which is stored as a bare string, without a relying party beside it — is read as belonging to it, so ceremonies in flight across the deploy still complete. New `Ssntpl\Neev\Services\RelyingPartyResolver`. See [docs/authentication.md](./docs/authentication.md#supported-domains)

### Changed

- **BREAKING: the API's passkey options endpoints are now `POST`** — `GET /neev/passkeys/register/options` becomes `POST /neev/passkeys/register/options` (no body), and `GET /neev/passkeys/login/options?email=…` becomes `POST /neev/passkeys/login/options` with `{"email": "…"}` in the body. The relying party is resolved from the request's `Origin`, and a browser omits that header on a same-origin GET, where it is a forbidden header name client JavaScript cannot add back — so as GETs these endpoints named no origin whenever the client and the API shared a host (the ordinary Blade layout, and any SPA deployed beside its API), fell back to `relying_party_id`, and the browser then refused the ceremony on the tenant's own domain. The only remedy was to move the API to a host of its own. As POSTs the header is always attached, so passkeys work on a custom domain in every deployment layout. The Blade starter kit's own options routes were already POST for this reason and are unchanged. Update any client that calls the API routes; a `GET` now returns `405`.
- **BREAKING: Magic-link authentication rebuilt as stateful, single-use tokens** — replaces the stateless signed-URL flow. Opaque, high-entropy tokens are stored **hashed** (SHA-256) in the new `magic_link_tokens` table; only the plain token ever leaves the app (in the emailed URL).
  ...
  - **Config-driven, extensible channels** — new `magic_link.channels` config (`web`, `mobile`, and any host-defined channel such as `desktop`, added by config with no code change). A channel with a `scheme`/`universal_link` is built as a deep link; otherwise the `EmailLinks::base()` host + `path`. Send accepts a `channel` parameter. A channel that cannot produce a usable URL throws `MagicLinkChannelException` (`422`) rather than falling through to a web link — either the channel is not declared, or it is a deep-link channel with `scheme` and `universal_link` both empty (the shipped `mobile` channel with `NEEV_MOBILE_SCHEME` unset is exactly this). Silently downgrading meant a client asking for `channel=mobile` got a `200` and a link that opened a browser instead of the app, with nothing logged.
  ...
  - **Unverified addresses keep working, unchanged** — a magic link is still mailed to an unverified address, and redeeming it still marks the email verified and fires `EmailVerified`. The link goes *to* that address, so following it proves inbox control exactly as the verification mail does. Gating sends on verification was considered and rejected: it removes the one path an unverified user has to become verified, stranding anyone who never received their verification mail.
  ...
  - Note: a magic link is a first factor only — an MFA-enrolled account is parked at the challenge, as it has been since 0.6.2.

### Fixed

- **Tenant-mode magic links land on a host that can redeem them** — `MagicLinkManager` took the link's host from a verified domain *owned by the resolved tenant* and fell back to `EmailLinks::base()`, the platform host. A tenant reached through a team-owned domain owns no domain record of its own, so its links were mailed to the platform host, where no tenant resolves, `TenantScope` narrows the lookup to `tenant_id IS NULL`, and the tenant-scoped token is never found — every such link was dead. The host is now, in order, the verified record the request resolved through, the tenant's own verified domain, or a verified domain of one of its teams
- **The web OAuth callback mails the first email MFA code** — it parked an email-MFA account at the challenge and relied on the Blade challenge page to issue the code when it opened. A headless install has no page of ours, so the account had nothing to answer with. The callback now issues and mails the code itself, as the API callback always did; the Blade page leaves a live code alone, so under the kit this is not a second mail. the OAuth and API challenge paths now share `AuthService::sendMfaEmailCode()`
- **A failed OAuth sign-up no longer throws headless** — the callback's failure branch redirected to `route('register')`, a Blade-kit route, so a headless install got `RouteNotFoundException` instead of a redirect. `EmailLinks::registerUrl()` points at the kit route or `{base}/register`
- **Magic-link replacement is actually serialized** — the delete-then-insert that supersedes a user's link took `lockForUpdate()` on the *user* row, but `User` carries `TenantScope`, so generation with no resolved tenant (CLI, queued jobs — the natural place to mail from) searched `tenant_id is null`, matched nothing for a tenant's user, and locked no row at all. The return value was discarded, so nothing noticed. Two concurrent sends could then both delete before either inserted, leaving two live links for one user and channel — the invariant the whole "generating a new link invalidates the previous one" guarantee rests on. The lock is now taken on the owner's `users` row through `DB::table()` — outside `TenantScope`, so it locks the row whatever tenant is resolved — and because that row always exists it serializes concurrent *first* issuance on MySQL and PostgreSQL alike (a `SELECT … FOR UPDATE` matching no token row would lock nothing on PostgreSQL). SQLite compiles `lockForUpdate()` to nothing and relies on its single-writer transaction.
- **`magic_link_tokens` indexes match how it is queried** — `expires_at` is now indexed: `neev:clean-magic-links` and `MagicLinkToken::scopeActive()` both range over it, and unindexed the cleaner full-scanned, slowest exactly when the table was largest and while contending with the insert every link request makes. The separate `user_id` index is dropped — `['user_id', 'channel']` covers it by leftmost prefix, so carrying both only added write cost on that same hot path. (MySQL still creates its own index for the foreign key, since the constraint is added before the composite index exists; the saving applies to the drivers that do not.)
- **Magic-link refusals no longer write stack traces to the log** — both `sendLoginLink` controllers passed the caught exception to `Log::warning($e)`, which renders the message *and* the full trace on a route that is unauthenticated and throttled only per IP, letting a caller decide what fills `laravel.log`. They now log a fixed message with safe context. The channel name, being client-supplied, goes in context rather than the message.
...
- **Two authenticators of the same model no longer overwrite each other's row** — registration now matches on `credential_id` scoped to the RP, which is the credential's unique cryptographic identity per the W3C WebAuthn spec. The previous match on `aaguid` was incorrect: AAGUID identifies an authenticator make/model shared by every unit of the same product line, so a user enrolling a second security key of the same model as their first lost the first one's row. A different key now always produces a different ID and always creates a new row. Re-enrolling the *same* key on the same RP would produce the same credential ID and update its row in place, but a browser no longer gets that far: registration options list the user's credentials on that relying party in `excludeCredentials`, so the authenticator refuses the ceremony and `navigator.credentials.create()` rejects with `InvalidStateError` before a request is made. The Blade stub catches it and tells the user the device is already enrolled; the same-ID update remains only as a backstop for clients that do not honour `excludeCredentials`

### Security

- **Magic-link issuance is capped per account** — every issuance invalidates the account's previous link for that channel, and `POST {prefix}/sendLoginLink` was throttled per IP only, so one client at ten requests a minute could keep a chosen address's link permanently dead and its inbox flooded, indefinitely, knowing nothing but the address. An account may now be issued `MagicLinkManager::ISSUANCE_LIMIT` (3) links per channel per `ISSUANCE_WINDOW` (5 minutes) — a hard invariant, not config. The refusal (`MagicLinkThrottledException`; `429` with `Retry-After` on the API, an error on the Blade form) happens before anything is invalidated, so the link the account already holds survives it. This bounds the abuse rather than removing it: within a window an attacker can still spend the budget ahead of the owner
- **The magic-link email no longer takes its host from the request** — the Blade flow built the emailed URL with `route()`, which derives its root from the `Host` header, discarding the URL `MagicLinkManager` had already built from configuration (and, in tenant mode, from the tenant's own DNS-verified domain). An unauthenticated `POST {prefix}/login/link` carrying a forged `Host` made the application mail the user a genuine, working single-use login token pointing at a host of the attacker's choosing — account takeover with no phishing infrastructure, since the mail is real. The controller now sends the manager's URL, and `channels.web.path` defaults to unset and follows the UI mode (`/login-link/verify` for the Blade kit, `/login-link` headless; override with `NEEV_MAGIC_LINK_WEB_PATH`). In tenant mode this was also a plain bug: `magic_link_tokens` is tenant-scoped, so a link on the wrong host never resolved its token
- **Passkey origin matching stays exact as the relying party becomes per-tenant** — origins were already matched exactly and subdomain matching was already off, but it was off by inheriting the library's default. Now that a ceremony's relying party varies per request, that is pinned as a [security invariant](./docs/design-principles.md): `RelyingPartyResolver::allowSubdomains()` returns `false` on every relying party, with no config key to loosen it. A passkey is bound to the relying party rather than to a single origin, so the origin list is the only thing standing between a compromised sibling host — a dangling CNAME, an XSS on a staging or marketing host, a tenant able to serve its own script from its subdomain — and an assertion accepted as the victim. On a multi-tenant installation a host under the platform domain is a *tenant's*, so "every host beneath it is mine" does not hold. Every admitted origin is named: your own operational hosts from `allowed_origins`, as before, and — this part is new — a context's own hosts from its verified `domains` rows, so a tenant on `acme.example.com` is admitted because its row says so rather than because the host ends in your platform domain, and one tenant's subdomain never admits a sibling. An installation where every host under the platform domain really is its own may widen the check by overriding `allowSubdomains()` in application code; that loosens the boundary beyond what any released version allowed, and is deliberately a code change rather than a flag. No action required on upgrade. See [docs/authentication.md](./docs/authentication.md#supported-domains)

## [0.6.3] - 2026-09-16

### Added

- **`POST {prefix}/mfa/otp/send` — resend the emailed MFA code** — the API had no way to ask for a second code. A client whose first one was lost in the post had to re-`POST {prefix}/login`, which opened a second login attempt and invalidated the MFA JWT it was already holding. The new endpoint sits in the same `neev:login`/`throttle:5,1` group as `/mfa/otp/verify` and takes the same short-lived JWT, so the code can only go to the address of whoever passed the first factor; it mints a fresh code and resets the guess budget, and answers 400 for an account with no active email factor. This is the API counterpart of the Blade kit's `otp.mfa.send`

### Fixed

- **`migrate:rollback` unwinds the package's tables** — two `down()` methods dropped things in an order the database refuses. `create_users_table` dropped `users` before `login_attempts`, which holds a foreign key to it (MySQL error 3730), and `create_teams_table` dropped the `default_team_id` index while the foreign key still needed it (error 1553). The teams migration now drops the foreign key, then the index, then the column — the one order both drivers accept, since MySQL keeps the index alive while the key needs it and SQLite keeps it alive after the column goes, so neither `dropConstrainedForeignId()` alone nor dropping the index first works on both. `migrate:fresh` was never affected; it drops tables directly without consulting `down()`

### Security

- **OAuth no longer skips MFA; a passkey and tenant/team SSO now satisfy it** — the MFA gate applied to password and magic-link login only, and the two remaining methods were each wrong in the opposite direction.
  **OAuth was a way around the second factor.** The callback logged an MFA-enrolled account straight in (web) or minted a full login token with `mfa_options: null` (API), so a compromised Google/GitHub/Microsoft/Apple account reached the account with no second factor — and on the web it then bounced the user between `NeevMiddleware` and a challenge page that had no account in the session to show it for. Whether the provider asked for MFA of its own is the provider's business and invisible here, so it earns no credit: OAuth now stops where a password login stops, with `auth_state: mfa_required` and the short-lived MFA JWT on the API and a redirect to the challenge page on the web. On a same-origin SPA monolith the login-token cookie the callback used to attach is now withheld until the challenge is answered and issued by `POST /otp/mfa` instead — it is a full login token, so handing it out before the second factor would have re-opened the same door one step further along.
  **A passkey was being challenged for a factor it had already supplied.** The ceremony runs with `userVerification: 'required'` and is validated with `CheckUserVerification`, so a successful login already proves possession of the authenticator *and* a local user check (biometric or device PIN) — that is the second factor, and asking for another one is asking the same question twice. An enrolled account that signed in with a passkey on the web was nonetheless parked at the challenge, while the API let it through; `NeevMiddleware` now reads a passkey attempt as answered, so both halves agree. See [docs/security.md](./docs/security.md#mfa-and-the-login-method)
  **Action required for OAuth installs:** users with active TOTP or email MFA will now be prompted for a second factor after signing in with a provider, and API clients must handle the `mfa_required` response from `POST {prefix}/oauth/{service}/callback`. **Tenant/team SSO is now exempt on both sides, which fixes a lockout.** The API side has always issued a full token without a challenge, on the grounds that the IdP owns that organization's authentication policy. The web side, being an ordinary session-based login, was gated by `NeevMiddleware` — and then stranded: `TenantSSOController::callback()` never sets `session('email')`, so the challenge page had no account to act on and bounced the user to the login screen. An MFA-enrolled member could not sign in through their organization's provider at all. The gate now reads an SSO attempt as answered, so both halves agree and the member gets in. Enforce a second factor at the identity provider if your organization requires one.
  **Where the web challenge lives.** The OAuth routes are registered kit or not, but the challenge page is the Blade kit's, so the redirect goes through the new `EmailLinks::mfaChallengeUrl()` rather than naming `otp.mfa.create` directly: a headless install (`ui` = `null`, the default) lands on `{base}/mfa-challenge/{method}` on its own frontend instead of failing on a route it never registered. `NeevMiddleware` uses the same method, so a parked session and a fresh callback agree on where to send the browser. On a stateful host that redirect also carries the **step-up JWT** in the auth cookie — the credential the API callback returns, good for the OTP step (`POST {prefix}/mfa/otp/verify` and `POST {prefix}/mfa/otp/send`) and nothing else — because a page the package does not ship needs something to answer the challenge with. It is swapped for the real login token once the code verifies, so the cookie never holds more authority than the factors answered so far
- **One convention for a login attempt's MFA state, and a gate that checks both halves of it** — the two halves of the codebase disagreed about what `login_attempts` means. The API named the factor at challenge time and left `is_success` false until the code verified; the web flow left `multi_factor_method` null and marked the attempt successful the moment the *first* factor passed, then used the null as its gate signal. One row shape therefore meant "waiting for a second factor" on one side and "signed in without one" on the other, and an abandoned web challenge was recorded as a successful login. `multi_factor_method` now uniformly names the second factor the login **demands**, written when the challenge opens, and `is_success` says whether the login **completed** — so `NeevMiddleware` checks both and either one closes the gate, rather than inferring the answer from a null. Checking both is also what carries sessions across the upgrade: a login parked at the challenge when the new code ships was written the old way round — successful from the first factor, with no `multi_factor_method` yet — and is challenged rather than let through on a half-updated row. `AuthService::login()` and `recordLoginAttempt()` take a `$pendingMfa` flag for the first factor to say it still owes a second; both new parameters are last and default to the previous behaviour.

## [0.6.2] - 2026-09-14

### Fixed

- **`neev:ui` no longer writes a parse error into `config/neev.php`** — the `'ui'` rewrite stopped at the first comma, so against the ordinary `env('NEEV_UI', 'blade')` it produced `'ui' => 'blade', 'blade'),`. It now rewrites only a single-line entry — `'ui'` at the start of its line, balanced brackets, no other key sharing the line, CRLF tolerated — and leaves anything else exactly as it is with a warning instead of guessing at it
- **The Blade sessions page and both logout-other-sessions actions read the session store's connection** — they queried `sessions` on the default connection; with `SESSION_CONNECTION` set they listed nothing and revoked nothing, since the driver writes elsewhere. All three now go through `AuthService::sessionsTable()`, `revokeOtherSessions()` and the new `revokeSession()`, which are public for applications with the same need

### Security

- **Magic links no longer skip MFA** — `GET {prefix}/loginUsingLink` and the Blade `login.link` route signed an MFA-enrolled account straight in. The API minted a full login token with `mfa_options: null`; the web flow signed the session in but never put the account where the challenge page looks for it, so the user bounced between the challenge and the login page. Both now stop where a password login stops — the API answers `auth_state: mfa_required` with the short-lived MFA JWT, the web flow redirects to the challenge page — and the password and magic-link paths now share one MFA hand-off per controller, which also refuses a deactivated account before recording an attempt or mailing a code
- **`createApiToken()` no longer promotes a scope list to `*` on a bare count match** — it compared `count($permissions)` with `Permission::count()`, so any list with the right *number* of entries became a wildcard, and so did an empty list on an install with no permissions registered. The comparison is by content now, and an empty list never collapses
- **Email-verification OTP guesses are reserved before the hash comparison** — the same read-then-increment overrun the MFA code had (requests arriving together all passed the cap check and all had their guess evaluated). The conditional increment now happens first, as in `HasMultiAuth::verifyMFAOTP()`
- **API token permissions are now enforceable** — `AccessToken` has carried a `permissions` column and a `can()` method since the beginning, and **nothing ever consulted either**, so a token scoped to `['read']` had exactly the same authority as one scoped to `['*']`. The docs advertised the feature. New opt-in `neev-token-can` middleware requires every ability listed (`neev-token-can:write`, `neev-token-can:read,write`), and the resolved token is now on the request as `neev.access_token` so application code can check `can()` without re-querying. `can()` also gained the distinction that makes it usable: a **login token** is minted by signing in with full credentials, so it carries the user's whole authority rather than a scope — `createLoginToken()` sets no permissions at all, so without this an ability check would have refused every ordinary signed-in caller. Only deliberately created API tokens are scoped, and a token row created directly without a `token_type` is now stored as an API token rather than inheriting the schema default of `login` — the unconditional kind cannot be acquired by omission. The middleware is opt-in, so nothing changes until a route attaches it
- **Changing a password now drops the account's other sessions and login tokens** — `AuthService::changePassword()` rewrote the hash and stopped there, so every session and login token minted under the old password stayed live. Changing a password after a compromise is the standard response to one, and it did not remove the attacker's access. Sessions are dropped for the database session driver (the only one where another session can be reached; on file/redis/cookie drivers attach Laravel's `AuthenticateSession` middleware, which invalidates a session whose stored password hash no longer matches). The session and login token making the request are spared, so the caller is not signed out mid-change. **API tokens are deliberately left alone** — they are credentials the user minted on purpose rather than a by-product of signing in, and killing a team's integrations is a product decision: `AuthService::revokeApiTokens()` and the existing `PasswordChanged` event are there for applications that want it
- **Domain verification is decided from the host, not from the request** (`POST {prefix}/tenant-domains`) — `type` was a client-supplied field, and `type: subdomain` set `verified_at` immediately with no check that the host had anything to do with this installation. Any team owner could therefore mark any domain verified. That is a takeover, not a cosmetic flaw: a verified claim reserves the domain installation-wide (no other team may then take it, so the rightful owner is locked out) and decides which team `@that-domain` signups are federated into. The decision now comes from the host and the claiming team, against the new `platform_domain` config: a tenant's subdomain is its slug, so team `acme` is issued `acme.otper.com` and that one claim is taken on trust, while everything else — another team's slug, one of your own operational hosts like `app.otper.com`, the apex, any outside domain — publishes the DNS TXT record. `type` is ignored entirely. Hosts are stored in one canonical spelling so the decision, the uniqueness reservation and the resolution lookup cannot disagree about `acme.otper.com.` versus `acme.otper.com`. `slug.reserved` gains the operational names that make this hold, and is where you reserve brand names you would not want in front of your domain. See [docs/configuration.md](./docs/configuration.md#platform-domains)
...
- **Email MFA codes are capped at 5 guesses, and a live code no longer has its life extended** — `multi_factor_auths` gains an `attempts` column, matching what the email-*verification* OTP has had since it shipped (`OTP::MAX_ATTEMPTS`). Before this, the MFA code — the one standing between a password holder and the account — counted nothing, while the verification code counted to five. Worse, `verifyMFAOTPCreate()` refreshed `expires_at` without minting a new code, so reopening the challenge page kept one six-digit secret alive indefinitely: unlimited guesses against a value that never rotated, behind only `throttle:10,1`. A live code now keeps its original deadline, an expired one is replaced, and every code-issuing path goes through `MultiFactorAuth::issueOtp()` so the counter cannot be reset in one place and forgotten in another. **The migration is edited in place** — existing installs add the column themselves: `$table->unsignedTinyInteger('attempts')->default(0);` on `multi_factor_auths`
- **Passkey web login no longer satisfies the MFA gate** — `PasskeyController::loginViaWeb()` passed its `LoginAttempt` as the fifth positional argument to `AuthService::login()`, which is the `$mfa` slot, leaving `$attempt` null. Eloquent's `Model::__toString()` JSON-encodes, so the model coerced silently into the string column: the real attempt was orphaned as a failure, a duplicate row was created, and `multi_factor_method` came out non-null — the very thing `NeevMiddleware` reads as proof the MFA challenge was answered, so a passkey login skipped it. The call now names the argument, and `$mfa`/`$attempt` on `login()` and `recordLoginAttempt()` are typed `?string` / `?LoginAttempt`
- **The MFA challenge page survives a lapsed session** — `GET /otp/mfa/{method}` read the account from `session('email')` and handed it straight to `User::findByEmail()`, whose parameter is a non-nullable string. Opening the page without a challenge in progress — a bookmark, a back button, a session that expired while the code was being fetched — raised a `TypeError` and returned 500. It now redirects to the login page with an explanatory error

## [0.6.1] - 2026-09-09

### Security

- **The Blade MFA challenge no longer trusts the request body** (`POST /otp/mfa`) — the route sits outside the authenticated group, as a login challenge must, and it resolved the account from `email` in the request before calling `AuthService::login()` without `viaRequestAuth`, i.e. a bare `Auth::login()` with no credential check. Anyone holding one second factor for an address could sign in as its owner with no password; because `verifyMFAOTP()` skips the enrolled-factor check for the `recovery` method, a single backup code was a complete standalone credential. The account now comes from `session('email')`, set by the password step, and `auth_method` must name a factor the account has enrolled
...
[Unreleased]: https://github.com/ssntpl/neev/compare/v0.6.3...HEAD
[0.6.3]: https://github.com/ssntpl/neev/compare/v0.6.2...v0.6.3
[0.6.2]: https://github.com/ssntpl/neev/compare/v0.6.1...v0.6.2
[0.6.1]: https://github.com/ssntpl/neev/compare/v0.6.0...v0.6.1
