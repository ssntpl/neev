...
- **`neev:ui` no longer writes a parse error into `config/neev.php`** — the `'ui'` rewrite stopped at the first comma, so against the ordinary `env('NEEV_UI', 'blade')` it produced `'ui' => 'blade', 'blade'),`. It now runs to the comma that ends the line, and a value it cannot parse is left alone with a warning instead of guessed at
- **The Blade sessions page and both logout-other-sessions actions read the session store's connection** — they queried `sessions` on the default connection; with `SESSION_CONNECTION` set they listed nothing and revoked nothing, since the driver writes elsewhere. All three now go through `AuthService::sessionsTable()`, `revokeOtherSessions()` and the new `revokeSession()`
