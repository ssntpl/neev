...
- **OAuth no longer skips MFA; a passkey now satisfies it** — the MFA gate applied to password and magic-link login only, and the two remaining methods were each wrong in the opposite direction.
  ...
  **Action required for OAuth installs:** users with active TOTP or email MFA will now be prompted for a second factor after signing in with a provider, and API clients must handle the `mfa_required` response from `POST {prefix}/oauth/{service}/callback`. Tenant/team SSO is deliberately unchanged: on the API side it stays outside the gate — the IdP owns that organization's authentication policy — while a web SSO session, which is an ordinary session-based login, is still subject to `NeevMiddleware` as it was before.
