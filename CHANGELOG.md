...
- **`passkeys.public_key`, `passkeys.aaguid`, `passkeys.transports` and `*_auth_settings.sso_client_id` are `string` rather than `text`** — indexable, and the values are short. Existing rows are unaffected, but note a `string` column caps at 255 characters
...
- **A domain could be claimed by two owners of the same kind** — the federate endpoints matched only within the team's own relation, so a domain another team held simply got a second row. Uniqueness is now per owner *type*: a tenant and a team may both federate one company domain, but two teams (or two tenants) may not, and only a **verified** claim reserves it. `Domain::isAvailable()` and `Domain::findByHostForOwnerType()` are the shared helpers. See [docs/teams.md](./docs/teams.md#who-may-claim-a-domain)
