...
- **BREAKING: Magic-link authentication rebuilt as stateful, single-use tokens** — replaces the stateless signed-URL flow. Opaque, high-entropy tokens are stored **hashed** (SHA-256) in the new `magic_link_tokens` table; only the plain token ever leaves the app (in the emailed URL).
  ...
  - **Config-driven, extensible channels** — new `magic_link.channels` config (`web`, `mobile`, and any host-defined channel such as `desktop`, added by config with no code change). A channel with a `scheme`/`universal_link` is built as a deep link; otherwise `base_url` + `path`. Send accepts a `channel` parameter. A channel that cannot produce a usable URL throws `MagicLinkChannelException` (`422`) rather than falling through to a web link — either the channel is not declared, or it is a deep-link channel with `scheme` and `universal_link` both empty (the shipped `mobile` channel with `NEEV_MOBILE_SCHEME` unset is exactly this). Silently downgrading meant a client asking for `channel=mobile` got a `200` and a link that opened a browser instead of the app, with nothing logged.
...
- **Magic-link replacement is actually serialized** — the delete-then-insert that supersedes a user's link took `lockForUpdate()` on the *user* row, but `User` carries `TenantScope`, so generation with no resolved tenant (CLI, queued jobs — the natural place to mail from) searched `tenant_id is null`, matched nothing for a tenant's user, and locked no row at all. The return value was discarded, so nothing noticed. Two concurrent sends could then both delete before either inserted, leaving two live links for one user and channel — the invariant the whole "generating a new link invalidates the previous one" guarantee rests on. The lock is now taken on the token rows themselves, under exactly the constraints `invalidatePrevious()` deletes by. On InnoDB the locking read over the `(user_id, channel)` index also gap-locks the range, so two concurrent first sends serialize too; PostgreSQL has no gap locks, so that one case is unserialized there.
- **`magic_link_tokens` indexes match how it is queried** — `expires_at` is now indexed: `neev:clean-magic-links` and `MagicLinkToken::scopeActive()` both range over it, and unindexed the cleaner full-scanned, slowest exactly when the table was largest and while contending with the insert every link request makes. The separate `user_id` index is dropped — `['user_id', 'channel']` covers it by leftmost prefix (the foreign key included), so carrying both only added write cost on that same hot path.
