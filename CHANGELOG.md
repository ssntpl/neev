...
- **Passkeys work on a tenant's own domain** — the relying party ID was one app-wide config value, and WebAuthn requires it to be the browser origin's host or a registrable suffix of it, so a tenant reaching the app on `acme.com` could not use passkeys at all: the browser refused the ceremony before a request was made, whatever the domain's verification status. It is now read off the request's context by one rule — the resolved tenant/team's **primary domain**, or its **first verified domain** when none is marked primary — and recorded against each credential in the new `passkeys.rp_id` column, since a passkey is bound to exactly one relying party for its lifetime. Login offers and accepts only the credentials belonging to the current one.
  ...
  **A tenant gets one relying party: its primary domain.** A tenant holding domains with different registrable roots cannot run a ceremony from the second one — no credential can span `ssntpl.in` and `otper.com`, whoever owns them — so a browser there is refused by WebAuthn itself. Subdomains are unaffected: a primary on `acme.com` covers `app.acme.com` and every host beneath it. Where passkeys are needed on more than one of a tenant's roots, the application chooses how — redirecting to the primary to sign in, serving `/.well-known/webauthn` for Related Origin Requests, or giving each domain its own context; the docs cover all three.
  A ceremony on a custom domain admits exactly that domain's origin, built from the record rather than the request, and forces `allow_origin_subdomains` off — so `allowed_origins` no longer has to list custom domains, and the host you serve passkeys from is the host to verify. Existing installs are untouched: the configured value still covers its own domain and every subdomain, credentials with a null `rp_id` are read as it, and the challenge is stored as it always was. New `Ssntpl\Neev\Services\RelyingPartyResolver`. See [docs/authentication.md](./docs/authentication.md#supported-domains)
...
- **Passkey origin checks no longer accept every subdomain** — both ceremonies constructed `CheckAllowedOrigins` with subdomain matching hardcoded on, so any host under `relying_party_id` could complete one. A passkey is bound to the relying party ID rather than to a single origin, so the origin list was the only thing standing between a compromised sibling subdomain — a takeover on a dangling CNAME, an XSS on a staging or marketing host, a tenant able to serve its own script from its subdomain — and an assertion accepted as the victim: request a challenge, run the ceremony from that origin, receive a session. Matching is now exact by default and governed by the new `neev.allow_origin_subdomains` key, which restores the old behaviour when every host under the domain is yours. The documentation already described the strict behaviour. See [docs/authentication.md](./docs/authentication.md#supported-domains)
