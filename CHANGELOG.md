...
- **OAuth no longer skips MFA; a passkey now satisfies it** — the MFA gate applied to password and magic-link login only, and the two remaining methods were each wrong in the opposite direction.
  ...
  **Where the web challenge lives.** The OAuth routes are registered kit or not, but the challenge page is the Blade kit's, so the redirect goes through the new `EmailLinks::mfaChallengeUrl()` rather than naming `otp.mfa.create` directly: a headless install (`ui` = `null`, the default) lands on `{base}/mfa-challenge/{method}` on its own frontend instead of failing on a route it never registered. `NeevMiddleware` uses the same method, so a parked session and a fresh callback agree on where to send the browser. Headless frontends should drive the API OAuth callback, which returns the step-up JWT instead of redirecting — the session-based verify endpoint ships with the kit
