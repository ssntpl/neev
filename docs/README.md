...
| [RFC 003 — Per-Group Auth Policies](./rfcs/003-auth-policies.md) | Accepted (§7 answered 2026-09-22) | One tenant, multiple user populations: policy-per-role/domain auth methods, identifier-first routing, platform policies as null-owner rows, migration from tenant/team auth settings. |
| [Email Reputation Package](https://github.com/ssntpl/email-reputation) | Moved out of this repo | Standalone classification-only package (free/disposable/relay/unknown), open for anyone to build; network tier deferred. Neev takes no dependency on it. |
