...
| A passkey is a complete second factor; a passkey login is never parked at the MFA challenge | 1 | The ceremony runs with `userVerification: 'required'` and is verified server-side, so it already proves possession of the authenticator *and* a local user check. Asking for a second factor afterwards asks the same question twice. Matches Apple, Google, Microsoft and the FIDO Alliance |
| Every passkey host is its own relying party, platform subdomains included | 1 | One relying party across a zone is one credential pool across it, and the exact-origin check cannot separate hosts that both legitimately answer to it. Accepted as a breaking change (users on subdomains re-enrol) rather than carried into a stable release |
| Managing API tokens requires a login token; no credential grants an ability it lacks | 1 | A scope that can rewrite itself is not a scope. The narrow rule is enforced in the controller, not on the route, so republishing `routes/neev.php` cannot drop it |
| A team invitation link carries a per-invitation secret | 1 | Redemption marks an address verified and assigns a role, so the proof it reached that inbox has to be unguessable. In-flight invitations break rather than stay forgeable |
