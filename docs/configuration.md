...
Origins permitted to complete WebAuthn ceremonies **under the relying party ID above**. List every allowed origin for multi-origin setups (e.g. app served from multiple domains), including each subdomain. Listing an origin the relying party ID does not cover has no effect; the browser rejects those ceremonies before the server sees them.

Subdomain matching is automatic: tenants on platform subdomains accept any host under the configured relying party; tenants on a verified custom domain accept only that exact origin. To disable subdomain matching on the platform path, override `allowSubdomains()` in a subclass of `RelyingPartyResolver` — see [Supported Domains](./authentication.md#supported-domains).
