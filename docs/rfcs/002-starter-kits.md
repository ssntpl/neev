...
|---|---|---|
| `email-verify` | `VerifyUserEmail` | `$url` (signed verification link), `$username`, `$purpose`, `$link_expiry` (minutes the link is valid), `$otp_expiry` (minutes the code is valid), `$otp` (verification code; null for reset/email-change purposes — the template chooses whether to show link, code, or both) |
